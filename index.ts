...
try {
  savedMessages = JSON.parse(
    await readFile('./savedMessages.json', 'utf-8'),
  ) as typeof savedMessages;
} catch (error) {
...
GlobalFonts.registerFromPath(
  join(import.meta.dirname, 'assets', 'fonts', 'FiraCodeNerdFont-Regular.ttf'),
  'FiraCodeNerdFont',
);
GlobalFonts.registerFromPath(
  join(import.meta.dirname, 'assets', 'fonts', 'SixHandsMarker.ttf'),
  'Six Hands Marker',
);
GlobalFonts.registerFromPath(
  join(import.meta.dirname, 'assets', 'fonts', '微軟正黑體.ttf'),
  'Microsoft JhengHei',
);
GlobalFonts.registerFromPath(
  join(import.meta.dirname, 'assets', 'fonts', 'NotoSansCJKtc-Regular.ttf'),
  'Noto Sans CJK TC',
);
...
const commands: Array<
  [
    string,
    SlashCommandBuilder | ContextMenuCommandBuilder | Object,
    ({
      interaction,
      savedMessages,
    }: {
...
> = [
  ...
  [
    ...
    async ({ interaction, savedMessages }) => {
      if (!interaction.isMessageContextMenuCommand()) return;
      const targetMessage = interaction.targetMessage;
    ...
    async ({ interaction, savedMessages }) => {
      ...
      if (textContents.length > 0) {
        component.addTextDisplayComponents(
          textContents.map((text) => new TextDisplayBuilder().setContent(text)),
        );
      }
      const urls = [
        ...attachments,
  ...
  [
    ...
    async ({ interaction }) => {
      ...
      function flatInnerComponents(
        components: TopLevelComponent | ComponentInContainer,
      ): ComponentInContainer[] {
        if (components.type === ComponentType.Container) {
          return Array.from(
            { length: components.components.length },
            (_, index) => flatInnerComponents(components.components[index]!),
          ).flat();
        }
        return [components];
      }

      function isThisComponentIsMediaGallery(
        component: ComponentInContainer,
      ): component is MediaGalleryComponent {
        return component.type === ComponentType.MediaGallery;
      }

      const imageUrls = [
        ...targetMessage.attachments.map((attachment) => attachment.url),
        ...targetMessage.components
          .flatMap((component) => flatInnerComponents(component))
          .filter((item) => isThisComponentIsMediaGallery(item))
          .flatMap((item) => item.items.map((i) => i.media.url)),
      ];
      ...
      for (const [, img] of Object.entries(urlImageMap)) {
        ...
        const widthRatio = singleImageMaxWidth / currentImageWidth;
        const heightRatio = singleImageMaxHeight / currentImageHeight;
        ...
        ctx.drawImage(
          img,
          xOffset + (singleImageMaxWidth - scaledWidth) / 2,
          yOffset + (singleImageMaxHeight - scaledHeight) / 2,
          scaledWidth,
          scaledHeight,
        );
      ...
      const lineWidth = Math.max(
        1,
        0.001 /*0.1%*/ * Math.min(maxHeight, maxWidth),
      );
      xOffset = 0;
      yOffset = 0;
      // Draw Line in X Direction
      for (let i = 0; i <= gridLike.length; i++) {
        ctx.moveTo(0, i * singleImageMaxHeight);
        ctx.lineTo(maxWidth, i * singleImageMaxHeight);
      }
      // Draw Line in Y Direction
      for (let i = 0; i <= gridLike![0]!.length; i++) {
        ctx.moveTo(i * singleImageMaxWidth, 0);
        ctx.lineTo(i * singleImageMaxWidth, maxHeight);
      }
    ...
    async ({ interaction }) => {
      ...
      ctx.fillText(
        ...
        (avatarPadding * 2 + avatarSize) / 2 -
          measureCtx.measureText(targetMessage.author.displayName || '').width /
            2,
        ...
        Math.min(
          ...
          Math.floor(
            (canvasWidth - (avatarPadding * 2 + avatarSize)) /
              wrappedLines.reduce((acc, line) => Math.max(acc, line.length), 0),
          ),
          // 16 * 3,
        ),
      );
...
async function exitHandler() {
  await writeFile('./savedMessages.json', JSON.stringify(savedMessages));
